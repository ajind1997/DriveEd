...
<script type='text/javascript'>
  ...
  // var obj = JSON.parse(o); // Parse JSON.

  var quizState = getCookie('quizState');
  if(isNullOrEmpty(quizState)) {
    quizState = generateNewQuizState(questions);
    setCookie('quizState', JSON.stringify(quizState));
  } else {
    try{
...
function buildQuizHTML(quizState, questions, currentQuestion, length) {
  $('#title').append('<h3>Feedback Mode</h3>');
...
function getPageLanguage() {
  ...
  function getNextQuestion(length) {
    ...
    $('#nextBtn').val('Next Question');
    // var random = Math.round(Math.random() * length);
    var url =  window.location.href;
    if(next==true)
        window.open(url, '_self');
    else{
        alert("Please submit an answer"); //submit answer before go to next question
    }
